classdef Violin_half < handle
    ...
    methods
        function obj = Violin_half(data, pos, varargin)
            ...
            % %%% end alp 11/9/21
            %%% alp added support for extra space 7/9/24
            args = obj.checkInputs(data, pos, varargin{:});
            centervalue = args.CenterSpace; %ALP 1/27/23 default is 0.03
            pos = pos + shift*centervalue;
...
%             jitter = 2*(rand(size(data))-0.5); %old
            ...
            obj.MedianPlot = scatter(boxpos, quartiles(2), args.MedianSize, [1 1 1], 'filled'); %alp 11/9/21 made x value boxpos
    ...
    methods (Access=private)
        function results = checkInputs(obj, data, pos, varargin)
            ...
            p.addParameter('ViolinColorMat', [], iscolor);
            ...
            p.addParameter('MedianSize', 45, isscalarnumber); %default copied from default value in function, added here for updating as desired ALP 1/27/23
            p.addParameter('CenterSpace', 0.03, isscalarnumber); %default space between violins, ALP 7/9/24
