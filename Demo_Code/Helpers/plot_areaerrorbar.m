...
%     figure(options.handle);
    ...
    patches = fill(options.ax, x_vector, [data_mean+error,fliplr(data_mean-error)], 'k');
    patches.FaceColor = options.color_area;
    set(patches, 'edgecolor', 'none');
    set(patches, 'FaceAlpha', options.alpha);
    hold on;
    plot(options.ax, options.x_axis, data_mean, 'color', options.color_line, ...
