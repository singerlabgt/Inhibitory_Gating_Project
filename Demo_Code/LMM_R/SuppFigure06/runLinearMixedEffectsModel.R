...
# 
foldN = "."
analysistypes = c("all", "high", "low", "no")
...
for (i in 1:length(analysistypes)){
  analysis_str <- analysistypes[i]
  tblFilename = paste0(foldN, "/TableData_resFR_allBinspost-", analysis_str, ".txt")
  txtFilename = paste0(foldN, "/stats_lme4_resFR_allBinspost-", analysis_str, ".txt") #create .txt file to save LMM outputs
