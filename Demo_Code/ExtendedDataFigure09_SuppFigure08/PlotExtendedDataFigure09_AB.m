...
for ct = 1:2         
    ...
    rate2plot = tempR(ismember(tempSess(:,1), params.goalshamMice) &...
        tempSess(:,6) ~= 1 & tempSess(:,8) == 1 & tempSess(:,9) == 1,:);
...
title(ax, 'PVxAi32 w/ Goal Stim in Nov')
text(ax, -8, 0.19, 'Pyr','Color','r');
text(ax, -8, 0.16, 'Narrow int','Color','b');
% save figure
figname = 'ExtendedDataFigure08_A_left';
savefigALP([figdir '/'], figname, 'filetype', 'pdf')


%PVxA32 with novel sham stim 
figure;
ax = axes('NextPlot','add','Box','off');
yline(ax,0,'k--')
ops.ax = ax;
...
for ct = 1:2         
    ...
    tempSess = table2array(tempSess);
    rate2plot = tempR(ismember(tempSess(:,1), params.goalshamMice) &...
        tempSess(:,6) ~= 1 & tempSess(:,8) == 1 & tempSess(:,9) == 2,:);
    rate2plot = (rate2plot - nanmean(rate2plot(:,1:21),2));
...
xlabel(ax, 'Time to SWR midpoint (s)')
ylabel(ax, 'Normalized \Delta firing from baseline')
title(ax, 'PVxAi32 w/ Sham Stim in Nov')
text(ax, -8, 0.19, 'Pyr','Color','r');
text(ax, -8, 0.16, 'Narrow int','Color','b');
% save figure
figname = 'ExtendedDataFigure08_A_right';
savefigALP([figdir '/'], figname, 'filetype', 'pdf')
...
figname = 'ExtendedDataFigure09_AB';
savefigALP([figdir '/'], figname, 'filetype', 'pdf')